...
# systemd-Dienst (gunicorn), Reverse-Proxy Caddy (Port 80), Backup-Cron.
...
#
# Optionales Argument: Pfad zu einer vorhandenen mobilfunk.db, die einmalig
# nach /var/lib/mobilfunk kopiert wird.
#
set -euo pipefail
...
log "1/8  Systempakete installieren"
...
log "5/8  Environment-Datei + Secret ($ENV_FILE)"
if [ ! -f "$ENV_FILE" ]; then
...
log "6/8  Dateirechte setzen"
chown -R "$APP_USER":"$APP_USER" "$APP_DIR" "$DATA_DIR"

log "7/8  systemd-Dienst 'mobilfunk-web'"
...
log "8/8  Reverse-Proxy Caddy (Port 80) + Backup-Cron"
