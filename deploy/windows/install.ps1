...
$HasBundledPython = Test-BundledPython $Backend
...
if ($HasBundledPython) {
  # Release-ZIP: Python samt allen Abhaengigkeiten liegt bereits fertig
  # unter backend\python-embed (siehe scripts/publish-windows-zip.sh) –
  # weder System-Python noch Internetzugang fuer pip noetig.
  $Python = Resolve-BackendPython $Backend
  Write-Host "Mitgeliefertes Python-Bundle gefunden – kein pip-Install noetig."
}
else {
  # Quellcode-Checkout (git clone): klassische .venv anlegen und befuellen.
